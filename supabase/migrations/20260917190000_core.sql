-- Núcleo Plan V, PostgreSQL 15+ / Supabase. Instalación nueva en esquema vacío.
-- Derivado del contrato 016 con vistas paciente y permisos verificados por pruebas SQL.
-- No reconcilia instalaciones legacy existentes; ver docs/ingreso-persistente.md.
...
-- PostgreSQL 15+: gen_random_uuid está disponible en pg_catalog.
do $$ begin
  if to_regclass('public.patients') is not null or to_regclass('public.profiles') is not null then
    raise exception 'Plan V core requires an empty application schema; existing installations need an explicit upgrade migration';
  end if;
end $$;
...
create view public.patient_access_view with (security_barrier = true) as
  select id,nutritionist_id,billing_status,billing_until
  from public.patients
  where deactivated_at is null and anonymized_at is null
    and (id = public.my_patient_id() or public.is_assigned_patient(id));
grant select on public.patient_access_view to authenticated;
