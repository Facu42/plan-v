...
**Alcance B (IA recetas + foto al aprobar):** el circuito borrador→asignar→registrar de PV-40 ya existía (biblioteca, wizard Manual/Asistente IA, asignar a día, «Registrar esta comida», card con macros). Lo que faltaba de verdad y se sumó acá: **la generación de foto del plato al aprobar (publicar) una receta**, que antes era un estado `cover_status='failed'` fijo desde el borrador — nunca se intentaba nada, y en modo persistente/staging directamente bloqueaba el guardado con 501. Ahora: tabla `recipe_covers` (mutable aparte de `recipe_versions`, que es inmutable una vez publicada), RPC `set_recipe_cover` con validación fail-closed (ready exige URL, los demás estados la prohíben — nunca se inventa), `publishRecipe` dispara `generateRecipeCoverImage` (`server/ai/recipe-cover.ts`, gateada por `AI_MODE`: en demo/disabled nunca llama al proveedor, sólo en `live`) justo después de aprobar, y `RecipeDishWell` ya renderiza la foto real cuando `cover_status='ready'`. El paciente también ve la portada resuelta en `list_assigned_recipes`. No se tocó el estado `macro_status='declared'` (sigue 501: fuera de alcance de este ticket). MVP: la imagen se guarda como `data:` URI (sin bucket de Storage propio todavía); si el volumen de recetas con foto crece, migrar a un bucket dedicado en vez del data URI inline.

**Alcance C (Progress):** ya resuelto por PV-43 (issue #9): `get_patient_progress` aplicado a plan-v-app y seed de Sofía con peso/cintura/cadera. `ShowroomProgress.tsx` ya maneja el estado vacío/error honestamente (`measurements_included=false`, 501 sin RPC). Nada nuevo acá, sólo verificación.

**Fail closed / sin inventar:** cover_status nunca se marca `ready` sin URL real devuelta por el proveedor; en modo demo/disabled/sin `OPENAI_API_KEY` queda `failed`, visible.

**Live:** migración `20260924120000_recipe_cover.sql` NO se aplicó a plan-v-app en esta pasada (el auto-mode de la sesión bloqueó el `apply_migration`, a diferencia de PV-43); queda para aplicar explícitamente. Sin eso, staging sigue sirviendo `cover_status` desde el fallback en memoria (siempre `none`), no desde la tabla nueva.

**Pendiente / fuera de esta pasada:** review Lumen/Iris del formato de card (sigue abierto desde PV-40); integrar el borrador de menú semanal (`generateMenuDraft`) al flujo de aprobación con el mismo patrón explícito draft→publish que recetas (hoy usa `save_meal_plan_draft`/`publish_meal_plan` de PV-19/20 directamente, funcional pero sin una UI de "aprobar menú IA" dedicada); bucket de Storage propio para portadas si el data URI inline no escala. Verificación: 848 pruebas OK, 2 omitidas (1 falla ajena, `supabase-client.test.ts`, por WebSocket nativo faltante en Node 20 de este sandbox — el repo pide Node ≥22); check, check:migrations, check:secrets y build OK.
