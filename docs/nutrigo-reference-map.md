...
| Progress (`105:2790`) | Superficie paciente Nutrigo operativa con adherencia, objetivo publicado, agua, sueño, energía, comidas revisadas, períodos 7/30/90 y comparativa del mismo paciente (medidas con fuente; sin ranking) | Fotografías corporales siguen aparte. **Contratos de layout 1440/800/390/320 y estados (PV-37) aterrizados; no es aprobación visual.** Live schema de progreso bloqueado si el proyecto tiene `patients` |
| Exercise (`105:2931`) | Superficie paciente Nutrigo operativa para actividad autodeclarada (alta, duración, intensidad, nota, resumen de siete días e historial) más biblioteca/detalle y rutina asignada sólo si hay habilitación verificada en servidor; la profesional asigna desde `/crm/ejercicio` | Live schema de `activity_logs`/biblioteca bloqueado si el proyecto tiene `patients`. Sin calorías inferidas ni rutina automática. **Contratos de layout 1440/800/390/320 (PV-37); no es aprobación visual** |
| Insights (`263:6588`) | Recursos paciente con seis guías operativas, artículos clínicos revisados sólo si están publicados y asignados, búsqueda por permisos y guardado de cuenta (guía/artículo/receta) | Live schema de `resources`/`favorites` bloqueado si el proyecto tiene `patients`. Sin publicación clínica automática ni imagen remota inventada. **Contratos de layout 1440/800/390/320 (PV-37); no es aprobación visual** |
| Insight Details (`279:9301`) | Detalle con autoría, revisión, licencia declarada, secciones, tags, relacionados, enlace profundo, compartir y lectura de asignaciones | Live persistencia bloqueada en el proyecto con `patients`. El paciente no ve borradores ni artículos sin asignar |
