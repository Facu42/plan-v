...
import weightMask from '../../assets/figma-mobile/427-14405-imgMaskGroup.svg';
import weightProgress from '../../assets/figma-mobile/427-14405-imgDonutProgress.svg';
import weightDivider from '../../assets/figma-mobile/427-14405-imgVector10.svg';
import calorieEllipse from '../../assets/figma-mobile/427-14405-imgEllipseBase.svg';
import calorieBase from '../../assets/figma-mobile/427-14405-imgDonutBase1.svg';
import calorieProgress from '../../assets/figma-mobile/427-14405-imgDonutProgress1.svg';
...
export function FigmaMobileDashboard({ patient: p, onNavigate }: Props) {
  ...
  const weightCeiling = weight ? Math.ceil(weight.value / 5) * 5 + 5 : null;
  const weightTicks = weightCeiling === null ? null : Array.from({ length: 5 }, (_, index) => weightCeiling - index * 5);
  const weightMarker = weightCeiling === null || weight === null ? 0 : Math.min(100, Math.max(0, (weightCeiling - weight.value) / 20 * 100));
  ...
  return <div className="fm-dashboard" data-figma-frame="427:14405">
    ...
    <section className="fm-statistics" aria-label="Resumen de salud" data-figma-node="427:14410">
      <button type="button" className="fm-stat fm-stat-weight" onClick={() => onNavigate('progreso')} data-figma-node="427:14411"><span className="fm-stat-head">Peso<span className="fm-stat-icon"><img src={speedometer} alt="" width="14" height="14" /></span></span><span className="fm-mini-weight-chart" data-figma-node="427:14416"><span className="fm-mini-weight-track">{weight && <span className="fm-mini-weight-marker" style={{ left: percent(weightMarker) }}><strong>{formatNumber(weight.value)}</strong><small>{weight.unit}</small></span>}</span><span className="fm-mini-weight-ruler" aria-hidden="true">{Array.from({ length: 21 }, (_, index) => <span key={index}><i />{index % 5 === 0 && <small>{weightTicks?.[index / 5] ?? '—'}</small>}</span>)}</span></span></button>
    ...
    <section className="fm-weight-widget fm-panel" data-figma-node="427:14552"><FigmaSectionHead title="Datos de peso" onOpen={() => onNavigate('progreso')} /><div className="fm-weight-chart" data-figma-node="427:14555"><img className="fm-weight-base" src={weightRing} alt="" /><img className="fm-weight-mask" src={weightMask} alt="" /><img className="fm-weight-progress" src={weightProgress} alt="" /><div><strong>{weight ? `${formatNumber(weight.value)} ${weight.unit}` : '—'}</strong><small>Peso declarado</small><small>Sin meta indicada</small></div></div><div className="fm-weight-note" data-figma-node="427:14569"><img src={weightDivider} alt="" /><p>{weight ? 'Tus medidas declaradas aparecen en Progreso.' : 'Cuando cargues una medida, aparecerá acá.'}</p></div></section>

    <section className="fm-calories-widget fm-panel" data-figma-node="427:14572"><FigmaSectionHead title="Registro de calorías" onOpen={() => onNavigate('diario')} /><div className="fm-calorie-ring" data-figma-node="427:14575"><img className="fm-calorie-ellipse" src={calorieEllipse} alt="" /><img className="fm-calorie-base" src={calorieBase} alt="" /><img className="fm-calorie-progress" src={calorieProgress} alt="" /><div className="fm-calorie-info"><img src={lightning} alt="" width="32" height="32" /><strong>{hasNutrition ? formatNumber(p.kcal) : '—'}<small> kcal</small></strong><span>{hasNutrition ? 'Calorías registradas' : 'Sin datos revisados'}</span></div></div><div className="fm-calorie-facts"><span><img src={fish} alt="" width="16" height="16" /><strong>{hasNutrition ? formatNumber(p.kcal) : '—'} kcal</strong><small>Comidas revisadas</small></span><span><img src={fire} alt="" width="16" height="16" /><strong>— kcal</strong><small>Actividad registrada</small></span></div><div className="fm-macro-list"><FigmaMacro label="Carbohidratos" value={hasNutrition ? p.macros.carbs_g : null} share={shares[1]} /><FigmaMacro label="Proteínas" value={hasNutrition ? p.macros.protein_g : null} share={shares[0]} /><FigmaMacro label="Grasas" value={hasNutrition ? p.macros.fat_g : null} share={shares[2]} /></div></section>
