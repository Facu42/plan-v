...
function CareRecordForm({patientId,kind,professional=false,onClose,onSaved}:{patientId:string;kind:Kind;professional?:boolean;onClose:()=>void;onSaved:()=>void}) {
  ...
  const [unit,setUnit]=useState(kind==='waist'?'cm':'kg');
  ...
  async function submit(e:FormEvent){e.preventDefault();if(lock.current)return;lock.current=true;setBusy(true);setStatus('');
    try{
      if(kind==='body_photo'){if(!image)throw new Error('Elegí una foto.');await careApi.photo(patientId,{id:id.current,recorded_on:date,image,note});}
        ...
        const origin = professional ? 'professional' as const : 'patient' as const;
        const data:CareData=kind==='weight'?{kind,value:Number(value),unit:unit as 'kg'|'lb',origin,note}:kind==='waist'?{kind,value:Number(value),unit:unit as 'cm'|'in',origin,note}:kind==='activity'?{kind,activity,minutes:Number(minutes),intensity,kcal:kcal===''?null:Number(kcal),note}:kind==='payment'?{kind,amount:Number(value),currency,method,reference,note}:{kind:'menu_request',target,reason,replacement};
        ...
        await careApi.save(patientId,pending.current,professional);
      }
      onSaved();
  ...
  return <div className="care-modal" role="dialog" aria-modal="true" aria-labelledby="care-form-title"><form className="care-form" onSubmit={submit}><header><h2 id="care-form-title">{CARE_LABELS[kind]}</h2><button type="button" aria-label="Cerrar formulario" disabled={busy} onClick={onClose}>×</button></header><fieldset disabled={busy || Boolean(pending.current)}><label>Fecha<input type="date" max={today()} value={date} onChange={e=>setDate(e.target.value)} required autoFocus/></label>
    {(kind==='weight'||kind==='waist'||kind==='payment')&&<div className="care-form-row"><label>{kind==='weight'?'Peso':kind==='waist'?'Cintura':'Importe'}<input type="number" step="0.01" min={kind==='waist'?10:kind==='weight'?1:0.01} max={kind==='waist'?300:kind==='weight'?500:100000000} value={value} onChange={e=>setValue(e.target.value)} required/></label>{(kind==='weight'||kind==='waist')&&<label>Unidad<select value={unit} onChange={e=>setUnit(e.target.value)}>{(kind==='weight'?['kg','lb']:['cm','in']).map(option=><option key={option}>{option}</option>)}</select></label>}</div>}
    {(kind==='weight'||kind==='waist')&&<p>Queda registrado como {professional?'carga del consultorio':'dato autodeclarado'}. No se estima desde fotos.</p>}
