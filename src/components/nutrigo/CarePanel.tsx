...
import { CARE_DOCUMENT_KIND_LABELS, CARE_DOCUMENT_KINDS, CARE_LABELS, MEASUREMENT_SOURCE_LABELS, careDateConstraintMessage, careInputSchema, describeCareRecord, describeMeasurement, dueCareReminders, isMeasurementKind, type CareData, type CarePreferences, type CareSnapshot } from '../../types/care';
...
const PROGRESS_KINDS = ['weight','waist','hip','body_photo','clinical_document'] as const;
function careActionLabel(kind: Kind) {
  ...
  if (kind === 'hip') return 'Registrar cadera';
  return 'Registrar actividad';
}
function measurementData(kind: 'weight' | 'waist' | 'hip', value: number, unit: string, source: 'patient' | 'professional', note: string): CareData {
  if (kind === 'weight') return { kind, value, unit: unit === 'lb' ? 'lb' : 'kg', source, note };
  return { kind, value, unit: unit === 'in' ? 'in' : 'cm', source, note };
}
function consentBlocked(kind: Kind, consented: string[]) {
  return isMeasurementKind(kind) && !consented.includes('measurement')
...
function CarePanelContent({patientId,mode}:{patientId:string;mode:Mode}) {
  ...
  return <section className="care-panel" aria-label={titles[mode]}>
    <header className="care-heading"><div><span className="care-eyebrow">{professional ? 'ACOMPAÑAMIENTO' : 'MI SEGUIMIENTO'}</span><h2>{titles[mode]}</h2><p>{professional ? 'Cargas del paciente, revisiones, estudios y pagos en una misma ficha. Las medidas se anotan a mano; las fotos no estiman peso ni contornos.' : mode==='activity' ? 'Duración, intensidad y calorías que registre tu dispositivo, si las tenés.' : mode==='menu' ? 'Pedí un reemplazo de receta o ingrediente. Tu nutricionista revisa la propuesta antes de compartirla.' : 'Peso, cintura y cadera opcionales, con fecha, unidad y origen. Fotos y estudios no se usan para estimar medidas. Compartidos sólo con tu nutricionista.'}</p></div><Icon name={professional ? 'contact' : 'trend'} size={24}/></header>
    ...
    {data && <>
      ...
      {(mode==='progress' || professional) && <div className="care-summary">{(['weight','waist','hip'] as const).map(kind => { const latest=data.measurements.filter(m=>m.kind===kind).sort((a,b)=>b.captured_on.localeCompare(a.captured_on) || b.created_at.localeCompare(a.created_at))[0]; return <article key={kind}><small>{CARE_LABELS[kind]}</small><strong>{latest ? describeMeasurement(latest) : 'Sin registro'}</strong><span>{latest ? `${latest.captured_on} · ${MEASUREMENT_SOURCE_LABELS[latest.source]}` : 'Opcional'}</span></article>; })}<article><small>{professional ? 'Por revisar' : 'Recordatorios'}</small><strong>{professional ? data.records.filter(r=>!r.reviewed_at).length : reminders.length}</strong><span>{professional ? 'Registros nuevos' : 'Pendientes de tu preferencia'}</span></article></div>}
      ...
      <div className="care-actions">
        {(mode==='progress' ? PROGRESS_KINDS : mode==='activity' ? ['activity'] as const : mode==='menu' ? ['menu_request'] as const : ['weight','waist','hip','payment'] as const).map(kind=><button className="nv-button primary" type="button" key={kind} onClick={()=>{setForm(kind);setStatus('');}} disabled={busy || consentBlocked(kind,data.consented)}><Icon name="plus" size={16}/>{careActionLabel(kind)}</button>)}
      </div>
...
{(mode==='menu'||professional) && <section className="care-recipes" aria-label="Reemplazos de menú"><h3>{professional?'Propuestas de reemplazo':'Alternativas revisadas'}</h3>{professional && !data.consented.includes('ai_menu_draft') && <p>El paciente debe habilitar el consentimiento de IA para preparar propuestas.</p>}{data.replacements.map(entry=><article key={entry.id}><span className="care-eyebrow">{entry.source==='demo'?'EJEMPLO DEMO · ':''}{entry.published_at?'COMPARTIDA CON EL PACIENTE':'BORRADOR PRIVADO'}</span><h3>{entry.recipe.title}</h3><p>{entry.recipe.explanation}</p><h4>Ingredientes</h4><ul>{entry.recipe.ingredients.map((i,index)=><li key={index}>{i}</li>)}</ul><h4>Preparación</h4><ol>{entry.recipe.steps.map((s,index)=><li key={index}>{s}</li>)}</ol>{professional && !entry.published_at && <CareRecipeEditor recipe={entry.recipe} busy={busy} onPublish={recipe=>void action(()=>careApi.publish(patientId,entry.id,entry.recipe,recipe),'Alternativa revisada y compartida.')} />}</article>)}{!data.replacements.length && <p>Todavía no hay alternativas compartidas.</p>}</section>}
      ...
      {(mode==='progress' || professional) && <section className="care-history" aria-label="Historial de medidas"><h3>Historial de medidas</h3><p>Sólo valores cargados a mano, con fecha, unidad y origen. Las fotos y los estudios no estiman peso ni contornos.</p>{data.measurements.length ? <ol>{data.measurements.map(entry => <li key={entry.id}><span className="care-eyebrow">{CARE_LABELS[entry.kind]} · <time>{entry.captured_on}</time></span><strong>{describeMeasurement(entry)}</strong></li>)}</ol> : <p className="care-empty">Todavía no hay medidas registradas.</p>}</section>}
    </>}
    {status && <p className="care-status" role="status">{status}</p>}
    {form && <CareRecordForm patientId={patientId} kind={form} professional={professional} onClose={()=>setForm(null)} onSaved={()=>{setForm(null);setStatus('Registro guardado y disponible en la ficha de tu nutricionista.');notifyCareChanged();}}/>}
...
function CareRecordForm({patientId,kind,professional=false,onClose,onSaved}:{patientId:string;kind:Kind;professional?:boolean;onClose:()=>void;onSaved:()=>void}) {
  ...
  const [unit,setUnit]=useState(kind==='weight'?'kg':'cm');
  ...
  async function submit(e:FormEvent){e.preventDefault();if(lock.current)return;lock.current=true;setBusy(true);setStatus('');
    try{
      if(kind==='body_photo'){if(!image)throw new Error('Elegí una foto.');await careApi.photo(patientId,{id:id.current,recorded_on:date,image,note});}
        ...
        const data:CareData=isMeasurementKind(kind)?measurementData(kind,Number(value),unit,professional?'professional':'patient',note):kind==='activity'?{kind,activity,minutes:Number(minutes),intensity,kcal:kcal===''?null:Number(kcal),note}:kind==='payment'?{kind,amount:Number(value),currency,method,reference,note}:{kind:'menu_request',target,reason,replacement};
  ...
  return <div className="care-modal" role="dialog" aria-modal="true" aria-labelledby="care-form-title"><form className="care-form" lang="es-AR" onSubmit={submit}><header><h2 id="care-form-title">{CARE_LABELS[kind]}</h2><button type="button" aria-label="Cerrar formulario" disabled={busy} onClick={onClose}>×</button></header><fieldset disabled={busy || Boolean(pending.current)}><label>Fecha<input type="date" max={today()} value={date} onChange={e=>{e.currentTarget.setCustomValidity('');setDate(e.target.value);setStatus('');}} onInvalid={e=>{e.preventDefault();setStatus(careDateConstraintMessage(e.currentTarget.validity));}} required autoFocus/></label>
    {(isMeasurementKind(kind)||kind==='payment')&&<div className="care-form-row"><label>{kind==='weight'?'Peso':kind==='waist'?'Cintura':kind==='hip'?'Cadera':'Importe'}<input type="number" step="0.01" min={kind==='waist'||kind==='hip'?10:kind==='weight'?1:0.01} max={kind==='waist'||kind==='hip'?300:kind==='weight'?500:100000000} value={value} onChange={e=>setValue(e.target.value)} required/></label>{isMeasurementKind(kind)?<label>Unidad<select value={unit} onChange={e=>setUnit(e.target.value)}>{kind==='weight'?<><option value="kg">kg</option><option value="lb">lb</option></>:<><option value="cm">cm</option><option value="in">in</option></>}</select></label>:<span/>}</div>}
    {isMeasurementKind(kind)&&<p>Origen: {professional?'profesional (consultorio)':'paciente (autodeclarado)'}. No se completa ni se estima desde una foto.</p>}
