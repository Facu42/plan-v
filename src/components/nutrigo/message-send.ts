type MessageInput = { patientId: string; text: string; role: 'patient' | 'pro'; id: string };
type MessageServices = {
  send: (patientId: string, text: string, from: 'patient' | 'vero', suggestedByAi: boolean, id: string) => Promise<unknown>;
...
export async function sendShowroomMessage(input: MessageInput, services: MessageServices): Promise<'sent' | 'sent-refresh-failed'> {
  ...
  await services.send(input.patientId, text, input.role === 'pro' ? 'vero' : 'patient', false, input.id);
  try {
