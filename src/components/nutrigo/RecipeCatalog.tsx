  ...
  return <section className="recipe-catalog" aria-label="Catálogo profesional de recetas">
    <header>
      ...
      <div className="recipe-header-actions">
        <NvButton className="nv-ghost" disabled={busy} onClick={() => { setPath('choose'); setEditing(null); setStatus(''); setError(''); }}>Nueva receta</NvButton>
    ...
    {(path === 'choose' || path === null) && <div className="recipe-choice" aria-label="Paso 1 de 2">
      <button type="button" onClick={() => { setPath('manual'); setEditing(emptyDraft()); }}>
        <strong>Carga manual</strong>
        <span>Ingredientes, cantidades y macros que declares. Nada se completa solo.</span>
      </button>
      <button type="button" onClick={() => { setPath('ai'); setEditing(null); }}>
        <strong>Asistente IA</strong>
        <span>Describí el plato. La propuesta queda para revisar antes de asignar.</span>
      </button>
    </div>}
    {path === 'ai' && <form className="recipe-form" onSubmit={(event) => {
      event.preventDefault();
      void run(async () => {
    ...
    {editing && <form className="recipe-form" onSubmit={submit}>
      ...
      <footer>
        ...
        <button type="button" className="recipe-cancel" disabled={busy} onClick={() => { setEditing(null); setPath(null); }}>Cerrar</button>
      </footer>
    </form>}
    <div className="recipe-grid">{recipes?.map((recipe) => {
      ...
      return <RecipePlateCard key={recipe.id} title={recipe.title} portions={recipe.current.yield_portions} card={card} actions={<>
        <NvButton className="nv-ghost" disabled={busy} onClick={() => { setEditing(fromRecipe(recipe)); setPath('manual'); setStatus(''); setError(''); }}>Editar</NvButton>
        {!recipe.current.published_at && <NvButton disabled={busy} onClick={() => void run(() => recipesApi.publish(recipe.id, recipe.current.version), 'Revisión publicada. El paciente la ve cuando la asignás.')}>Publicar</NvButton>}
        {recipe.published && <NvButton disabled={busy} onClick={() => { setAssigning(recipe); setStatus(''); setError(''); }}>Asignar</NvButton>}
      </>} />;
    })}</div>
    {assigning?.published && <div className="recipe-overlay" role="dialog" aria-label="Así lo ve tu asesorado">
      <div className="recipe-overlay-card">
        ...
        <footer className="recipe-actions">
          <NvButton disabled={busy} onClick={() => void run(async () => {
            await recipesApi.assignDay(assigning.id, { patient_id: patientId, expected_version: assigning.published!.version, for_date: day, slot });
            setAssigning(null);
          }, 'Asignada al día. El paciente puede registrarla.')}>Confirmar asignación</NvButton>
          <button type="button" className="recipe-cancel" onClick={() => setAssigning(null)}>Cerrar</button>
        </footer>
      </div>
    </div>}
  </section>;
