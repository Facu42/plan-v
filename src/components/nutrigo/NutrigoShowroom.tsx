...
export function NutrigoShowroom({ darkMode, onToggleTheme, lockedRole = null, allowRoleSwitch = false, onSignOut }: NutrigoShowroomProps) {
  ...
  const confirmAppointment = async (confirmation: 'attending' | 'needs_change') => {
    if (!selected) return;
    const result = await api.confirmAppointment(selected.id, confirmation);
    addPatient(result.patient);
  };
  useEffect(() => {
    if (!moreOpen) return;
