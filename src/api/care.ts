...
export const careApi = {
  ...
  document: (id: string, input: {id:string;recorded_on:string;file:string;note:string;filename:string;document_kind:string}) => request(`${base(id)}/documents`, json('POST', input)),
  openDocument: (id: string, recordId: string) => request<{url:string;expires_in:number;mime:string;filename:string}>(`${base(id)}/documents/${recordId}`),
  deleteDocument: (id:string,recordId:string)=>request(`${base(id)}/documents/${recordId}`,{method:'DELETE'}),
